...
// considering the convention, class names should start with capital letters
...
class Solution {
    alphanumeric[] sortedStrings(int n, String a[]) {
        ...
        for (String arrangement : a) {
            tracker.put(arrangement, tracker.getOrDefault(arrangement, 0) + 1);
        }
        int size = tracker.size();
        alphanumeric[] result = new alphanumeric[size];
        int index = 0;
        for (String key : tracker.keySet()) {
            int count = tracker.get(key);
            result[index++] = new alphanumeric(key, count);
        }
        return result;
    }
}