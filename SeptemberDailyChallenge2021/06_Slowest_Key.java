...
class Solution {
    public char slowestKey(int[] releaseTimes, String keysPressed) {
        ...
        for (int i=1; i<length; i++) {
            ...
            if (currentPress > longestPress) {
                longestPress = currentPress;
                longPressIndex = i;
            }
            else if (currentPress == longestPress && keysPressed.charAt(i) > keysPressed.charAt(longPressIndex)) {
                longPressIndex = i;
            }
        }
        return keysPressed.charAt(longPressIndex);
    }
}
...
class Solution {
    public char slowestKey(int[] releaseTimes, String keysPressed) {
        ...
        int indexForMaxKeyPress = 0;
        for (int i=1; i<length; i++) {
            char currentKey = keysPressed.charAt(i);
            int time = releaseTimes[i] - releaseTimes[i - 1];
            duration[currentKey - 'a'] = Math.max(duration[currentKey - 'a'], time);
        }
        int maxKeyPress = 0;
        for (int i=25; i>=0; i--) {
            if (maxKeyPress < duration[i]) {
                maxKeyPress = duration[i];
                indexForMaxKeyPress = i;
            }
        }
        return (char)(indexForMaxKeyPress + 'a');
    }
}